...
{{
  ...
  "risk_score": 1-10 as an integer. 1 = low risk / most promising, 10 = high risk / avoid. Consider: if orange and competitor is thriving, that's high risk (crowded). If orange and competitor died/pivoted, that could be medium risk (idea may be right, execution was wrong -- worth studying why). If black and shelf life is temporal, that's lower risk (real opportunity). If black and shelf life is structural, that's high risk (probably won't work for a real reason).
  "risk_reasoning": "2-3 sentences explaining the score, referencing what the search evidence actually showed"
}}"""
...
def rebuild_top_ideas(all_scored):
    ...
    for i, r in enumerate(all_scored, 1):
        lines.append(
            ...
            f"{r['competitor_status']} | {r['risk_score']}/10 | {r['risk_reasoning'][:150]} |"
        )
